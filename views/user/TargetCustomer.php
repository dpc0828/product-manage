    ...
    <div class="padding-2">
        ...
        <form class="layui-form narrow_2" action="">
            <div class="layui-form-item" style="width: 33rem;">
                <select id="select_target" lay-filter="target_template" lay-search>
        ...
        <form class="layui-form narrow_2" id="target" action="javascript:;" lay-filter="target">
            ...
            <div class=" text-center">
                <button class="layui-btn layui-btn-primary" onclick="saveTargetTemplate(0)">保存为模板</button>
                <button class="layui-btn" id="issue_btn" onclick="issueTarget()">立即提交</button>
            </div>
        </form>
    </div>
    <script src="/js/user/user.js?v=<?php echo \app\utils\Utils::version(); ?>" type="text/javascript"></script>
    <script>
        <?php if(!empty($info['customer_setting'])) { ?>
        layui.use('form', function(){
            var form = layui.form;
            form.val("target", {
                //性别
                "sex": "<?php if($info['customer_setting']['gender'] == 1) {echo 1;}?>",
                "Sex[0]": "<?php if(!empty($info['customer_setting']['male_percent'])) {echo $info['customer_setting']['male_percent'] * 100;}?>",
                "Sex[1]": "<?php if(!empty($info['customer_setting']['female_percent'])) {echo $info['customer_setting']['female_percent'] * 100;}?>",
                //年龄
                "age": "<?php if($info['customer_setting']['age'] == 1) {echo 1;}?>",
                "Age[younger]": "<?php if(!empty($info['customer_setting']['age18_24'])) {echo $info['customer_setting']['age18_24'] * 100;}?>",
                "Age[middle]": "<?php if(!empty($info['customer_setting']['age25_33'])) {echo $info['customer_setting']['age25_33'] * 100;}?>",
                "Age[older]": "<?php if(!empty($info['customer_setting']['age34_50'])) {echo $info['customer_setting']['age34_50'] * 100;}?>",
                //地域
                // "territory": "",
                // "Territory[0]": "",
                // "Territory[1]": "",
                ...
                <?php
                if(!empty($info['customer_setting']['exclude_province'])) {
                    $exclude_province = explode(' ', $info['customer_setting']['exclude_province']);
