			...
			<div class="col-xs-12">
				<form class="form-horizontal" autocomplete="off" id="validation-form" novalidate="novalidate" method="post" action="">
					<div class="form-group">
						...
						<div class="col-sm-9">
							<div class="clearfix">
								<input type="text" name="name" value="<?php echo $info['name']; ?>" id="name"  placeholder="角色名称" aria-required="true" aria-invalid="true" class="col-xs-10 col-sm-5" />
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<div class="clearfix">
								<input type="text" name="remark" value="<?php echo $info['remark']; ?>" id="remark" placeholder="角色描述" class="col-xs-10 col-sm-5" />
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<div class="radio">
								<label>   
									<input name="status" <?php if($info['status'] == 1) { ?> checked="checked" <?php } ?> value="1" type="radio" class="ace status" />
									<span class="lbl"> 启用</span>
								</label>
							</div>
							<div class="radio">
								<label>
									<input name="status" <?php if($info['status'] == 2) { ?> checked="checked" <?php } ?> value="2" type="radio" class="ace status" />
		...
		jQuery(function($) {
			$('#validation-form').validate({
				...
				submitHandler: function (form) {
					...
					g.func.ajaxPost("/admin/operator/edit-role?id=<?php echo $info['id']; ?>", param , function(res){
